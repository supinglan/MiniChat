...
DWORD WINAPI ThreadFunc(LPVOID lpParam) {
    ...
    std::cout << "socket id is " << tcps << "." << std::endl;
    //发送回复报文给客户端
    std::cout << "Serve client " << inet_ntoa(clientaddr.sin_addr) << ":"
              << ntohs(clientaddr.sin_port) << std::endl;
    ResPacket rspns = {OK,
                       "Welcome to Mini Chat Server!\n"
    };
    if (!((Client *) lpParam)->SendRes(&rspns)) {
        return 0;
    }
    //循环获取客户端命令报文并进行处理
    for (;;) {
    ...
    closesocket(tcps);
    delete lpParam;
    std::cout << "Close the connection to client "
...
int main() {
    ...
    if (!Init(&listenSock))
        return 1;
    std::cout << "Server listening on " << SERVER_PORT << " port..." << std::endl;
...
int Init(SOCKET *listenSock) {
    ...
    if (err != 0) {
        std::cout << "Winsock初始化时发生错误!" << std::endl;
        return 0;
    }
    if (LOBYTE(wsaData.wVersion) != 2 || HIBYTE(wsaData.wVersion) != 2) {
        WSACleanup();
        std::cout << "无效Winsock版本!" << std::endl;
        return 0;
    }
    ...
    if (err != 0) {
        err = WSAGetLastError();
        WSACleanup();
        std::cout << "Socket绑定时发生错误!" << std::endl;
        return 0;
    }
    err = listen(tcps_listen, 3);
    if (err != 0) {
        WSACleanup();
        std::cout << "Socket监听时发生错误!" << std::endl;
        return 0;
    }
